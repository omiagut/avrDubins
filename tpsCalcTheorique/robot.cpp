...
*/
void Robot::moveTheorique(int phase, const unsigned int step) {
...
void Robot::setOCRnA(const int phase) {
    ...
    arrondi(_ocr0A);
    arrondi(_ocr2A);
    vitessesRoues[phase].ocr0A = (unsigned short) _ocr0A;
    vitessesRoues[phase].ocr2A = (unsigned short) _ocr2A;
}

/*
 * nombre de tics théorique des encodeurs / seconde
 * Distance parcourue à la seconde / circonférence * nb de tics sur un tour
 */
void Robot::setTicsSeconde(const int phase) {
    float distanceSeconde;

    /* roue droite */
    distanceSeconde = vitessesRoues[phase].roueDroite / 60.0;
    vitessesRoues[phase].ticsDroite = (unsigned int) distanceSeconde / (DIAMETRE * PI) * TICS;

    /* roue gauche */
    distanceSeconde = vitessesRoues[phase].roueGauche / 60.0;
    vitessesRoues[phase].ticsGauche =  (unsigned int) distanceSeconde / (DIAMETRE * PI) * TICS;
}
